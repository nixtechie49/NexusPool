...
namespace nexuspool
{
	Daemon_connection::Daemon_connection(network::Socket::Sptr&& socket, 
		...
		, m_maintenance_timer{std::move(maintenance_timer)}
	{
	}
	...
	void Daemon_connection::process_data(network::Shared_payload&& receive_buffer)
	{
		...
		if (packet.m_header == Daemon_packet::GOOD)
		{
			m_logger->info("Daemon: Block {} accepted.", Core::hashBlockSubmission.ToString().substr(0, 20));
			Core::NewRound();
		...
		//		Core::hashBlockSubmission = CONNECTIONS[nIndex]->SUBMISSION_BLOCK->GetHash();
			...
			//			Core::LAST_ROUND_BLOCKFINDER = CONNECTIONS[nIndex]->ADDRESS;
						...
						m_logger->info("Daemon: Submitting Block for Address {0} on Connection: {1}", 
										pool_connection->get_nxs_address(), pool_connection->get_remote_address());
					}
					else
					{
						m_logger->info("Daemon: Stale Block Submitted on Connection: {}", pool_connection->get_remote_address());
					}
		...
		/** Get Blocks if Requested From Pool Connection. **/
		{
			...
			for(auto& pool_data : m_pool_connections)
			{
				while(pool_data.m_blocks_requested > 0)
				{
					...
					m_logger->debug("Daemon: Requesting Block from wallet. Connection has {} block requests left", pool_data.m_blocks_requested);
				}
			}
		}
		//}
